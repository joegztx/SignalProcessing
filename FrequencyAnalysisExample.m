...
% phase of an FFT? Is my signal periodic? How do I measure power? Is there
% one, or more than one signal in this band?
%
...
% real part, $x_r$, and an imaginary part, $x_i$, such that $x = x_r +
% ix_i$. The magnitude of $x$ is computed as $\sqrt{(x_r^2+x_i^2)}$, and
% the phase of $x$ is computed as $\arctan{(x_i/x_r)}$. You can use MATLAB
